...
export class GroepComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  exportToPdf() {
    ...
    if (this.myTimesheetsItems.length > 0) {
      ...
      for (let i = 0; i < this.myTimesheetsItems.length; i++) {
        rowsExtra.push([
          this.myTimesheetsItems[i].datum,
          this.myTimesheetsItems[i].beginTijd,
  ...
  ngAfterViewInit() {
    ...
    this._groepservice
      .getFilesByGroep(this.groepNaam)
      .subscribe(res => (this.uploadedFiles = res as Files[]));
  }
  ...
  onUpload(event) {
    ...
    let file = new Files(
      ...
      event.files[0].size,
      event.files[0].type,
      event.files[0].webkitRelativePath,
      this.groepNaam
    );

    this._groepservice
      .uploadFile(file)
      .subscribe(() => this.uploadedFiles.push(file));
  }
  openTimesheets() {
    ...
    this.fileNameDialogRefTimesheets.afterClosed().subscribe(event => {
      if (!isNullOrUndefined(event)) {
        {
          if (!isNullOrUndefined(event[0])) {
            this.addTimesheet(event[0], event[1], event[2], event[3]);
          }
        }
      }
    });
  ...
  addTimesheet(date, begin, einde, event) {
    ...
    if (this.newTimesheet.eindTijd != null) {
      this.myTimesheetsItems.push(this.newTimesheet);
    }
  }

  onRowSelect(event) {
    window.location.href =
      "http://46.101.57.64:1337/download/" + event + "/" + this.groepNaam;
  }
  openDialog() {
    ...
    this.fileNameDialogRef.afterClosed().subscribe(event => {
      if (!isNullOrUndefined(event)) {
        if (!isNullOrUndefined(event[0])) {
          this.addAgenda(event[0], event[1], event[2]);
        }
      }
    });
  }
  ...
  deleteAgendaItem(id: number) {
    ...
    this._groepservice.deleteAgendaItem(id).subscribe();
  }
  ...
  ngOnInit() {
      ...
      if (data.user.user != LoginServiceApi.username)
        this.pushNotification.show(
          this.groepNaam,
          {
            body: data.msg.msg,
            icon:
              "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSL4T3mP7R5utqlNz6ZArDagjg-9F0It0DG1REv0tWgSTxPEypz"
          },
          3000 // close delay
        );
    });
    ...
    this._groepservice.getStudentsByGroep(this.groepNaam).subscribe(data => {
      this.myData2 = data;
    });
    this._groepservice.getAgendaByGroep(this.groepNaam).subscribe(data => {
    ...
    this._groepservice.getChatByGroup(this.groepNaam).subscribe(data => {
